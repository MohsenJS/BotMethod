...
class BotMethod
{
     ...
     * @param bool       $disable_notification Pass True, if it is not necessary
     ...
     * @param string     $sticker_set_name Name of the sticker set to be set as
     *                                     the group sticker set
     *
     * @return object
     */
     ...
     *                                  answered
     * @param string $text              Text of the notification. If not
     *                                  specified, nothing will be shown to the user, 0-200 characters
     * @param bool   $show_alert        If true, an alert will be shown by the
     *                                  client instead of a notification at the top of the chat screen. Defaults
     *                                  to false.
     * @param string $url               URL that will be opened by the user's
     *                                  client.
     * @param int    $cache_time        The maximum amount of time in seconds
     *                                  that the result of the callback query may be cached client-side. Telegram
     ...
     *
     * @param string $inline_query_id     Unique identifier for the answered
     *                                    query
     * @param json   $results             A JSON-serialized array of results for
     *                                    the inline query
     * @param int    $cache_time          The maximum amount of time in seconds
     *                                    that the result of the inline query may be cached on the server. Defaults
     *                                    to 300.
     * @param bool   $is_personal         Pass True, if results may be cached on
     *                                    the server side only for the user that sent the query. By default,
     *                                    results may be returned to any user who sends the same query
     * @param string $next_offset         Pass the offset that a client should
     *                                    send in the next query with the same text to receive more results. Pass
     *                                    an empty string if there are no more results or if you don‘t support
     *                                    pagination. Offset length can’t exceed 64 bytes.
     * @param string $switch_pm_text      If passed, clients will display a
     *                                    button with specified text that switches the user to a private chat with
     *                                    the bot and sends the bot a start message with the parameter
     *                                    switch_pm_parameter
     * @param string $switch_pm_parameter Deep-linking parameter for the /start
     ...
     * @param int|string $chat_id                  Unique identifier for the target chat or username of the target
     *                                             channel (in the format @channelusername)
     * @param int        $message_id               Required if inline_message_id
     *                                             is not specified. Identifier of the sent message
     * @param string     $text                     New text of the message
     * @param string     $inline_message_id        Required if chat_id and
     *                                             message_id are not specified. Identifier of the inline message
     * @param json       $reply_markup             Additional interface options. A JSON-serialized object for an
     *                                             inline keyboard, custom reply keyboard, instructions
     ...
     *                                      JSON-serialized object for an inline keyboard, custom reply keyboard,
     *                                      instructions to remove reply keyboard or to force a reply from the user.
     * @param string     $parse_mode        Send Markdown or HTML, if you want
     ...
     * @param int        $message_id        Required if inline_message_id is not
     *                                      specified. Identifier of the sent message
     * @param json       $media             A JSON-serialized object for a new
     ...
     * @param int|string $chat_id      Unique identifier for the target chat or
     *                                 username of the target channel (in the format @channelusername)
     * @param int        $message_id   Identifier of the original message with
     *                                 the poll
     * @param json       $reply_markup A JSON-serialized object for a new
     ...
     * @param int|string $chat_id              Unique identifier for the target
     *                                         chat or username of the target channel (in the format @channelusername)
     * @param string     $sticker              Sticker to send. Pass a file_id
     *                                         as String to send a file that exists on the Telegram servers
     *                                         (recommended), pass an HTTP URL as a String for Telegram to get a .webp
     ...
     * @param file $png_sticker Png image with the sticker, must be up to 512
     ...
     *
     * @param int    $user_id        User identifier of sticker file owner
     * @param string $name           Short name of sticker set, to be used in
     *                               t.me/addstickers/ URLs (e.g., animals)
     * @param string $title          Sticker set title, 1-64 characters
     * @param file   $png_sticker    Png image with the sticker, must be up to
     *                               512 kilobytes in size, dimensions must not exceed 512px, and either width
     *                               or height must be exactly 512px
     * @param string $emojis         One or more emoji corresponding to the
     *                               sticker
     * @param bool   $contains_masks Pass True, if a set of mask stickers should
     *                               be created
     * @param json   $mask_position  A JSON-serialized object for position where
     ...
     *
     * @param int    $chat_id                       Unique identifier for the
     *                                              target private chat
     * @param string $title                         Product name, 1-32
     *                                              characters
     * @param string $description                   Product description, 1-255
     *                                              characters
     * @param string $payload                       Bot-defined invoice payload,
     *                                              1-128 bytes. This will not be displayed to the user, use for your
     *                                              internal processes.
     * @param string $provider_token                Payments provider token,
     *                                              obtained via Botfather
     * @param string $start_parameter               Unique deep-linking
     *                                              parameter that can be used to generate this invoice when
     *                                              used as a start parameter
     * @param string $currency                      Three-letter ISO 4217
     *                                              currency code
     * @param array  $prices                        Price breakdown, a list of
     *                                              components (e.g. product price, tax, discount, delivery cost,
     *                                              delivery tax, bonus, etc.
     * @param int    $reply_to_message_id           If the message is a reply,
     *                                              ID of the original message
     * @param json   $reply_markup                  Additional interface
     *                                              options. A JSON-serialized object for an inline keyboard,
     *                                              custom reply keyboard, instructions to remove reply
     *                                              keyboard or to force a reply from the user.
     * @param string $provider_data                 JSON-encoded data about the
     ...
     *                                              provider.
     * @param string $photo_url                     URL of the product photo for
     *                                              the invoice
     * @param int    $photo_size                    Photo size
     * @param int    $photo_width                   Photo width
     * @param int    $photo_height                  Photo height
     * @param bool   $need_name                     Pass True, if you require
     *                                              the user's full name to complete the order
     * @param bool   $need_phone_number             Pass True, if you require
     *                                              the user's phone number to complete the order
     * @param bool   $need_email                    Pass True, if you require
     *                                              the user's email address to complete the order
     * @param bool   $need_shipping_address         Pass True, if you require
     *                                              the user's shipping address to complete the order
     * @param bool   $send_phone_number_to_provider Pass True, if user's phone
     *                                              number should be sent to provider
     * @param bool   $send_email_to_provider        Pass True, if user's email
     *                                              address should be sent to provider
     * @param bool   $is_flexible                   Pass True, if the final
     ...
     * queries.
     *
     * @param string $shipping_query_id Unique identifier for the query to be
     *                                  answered
     * @param bool   $ok                Specify True if delivery to the
     *                                  specified address is possible and False if there are any problems (for
     *                                  example, if delivery to the specified address is not possible)
     * @param json   $shipping_options  Required if ok is True. A
     *                                  JSON-serialized array of available shipping options. 512px
     * @param string $error_message     Required if ok is False. Error message
     *                                  in human readable form that explains why it is impossible to complete the
     ...
     * queries.
     *
     * @param string $pre_checkout_query_id Unique identifier for the query to
     *                                      be answered
     * @param bool   $ok                    Specify True if everything is
     *                                      alright (goods are available, etc.) and the bot is ready to proceed with
     *                                      the order. Use False if there are any problems.
     * @param string $error_message         Required if ok is False. Error
     *                                      message in human readable form that explains the reason for failure to
     *                                      proceed with the checkout (e.g. "Sorry, somebody just bought the last of
     *                                      our amazing black T-shirts while you were busy filling out your payment
     ...
     * @param string $game_short_name      Short name of the game, serves as the
     *                                     unique identifier for the game.
     * @param int    $reply_to_message_id  If the message is a reply, ID of the
     *                                     original message
     * @param json   $reply_markup         A JSON-serialized object for an
     *                                     inline keyboard. If empty, one ‘Play game_title’ button will be shown. If
     ...
     * @param bool   $force                Pass True, if the high score is
     *                                     allowed to decrease. This can be useful when fixing mistakes or banning
     *                                     cheaters
     * @param bool   $disable_edit_message Pass True, if the game message should
     *                                     not be automatically edited to include the current scoreboard
     * @param int    $chat_id              Required if inline_message_id is not
